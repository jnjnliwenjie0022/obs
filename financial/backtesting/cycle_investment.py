...
#df = yf.download('2317.TW', start = '2001-01-01')
#df = yf.download('2882.TW', start = '2008-01-01')
#df = yf.download('^TWII ')
...
data = pd.json_normalize(data['line']['34']['data'])
...
def l_period_fun(row):
    ...
    if(L_diff1 > 0 and L_diff2 > 0):
        ...
        return 3
data['L_period'] = data.apply(l_period_fun, axis=1)
...
fig.append_trace(go.Scatter(x = df.index, y = df['L_period']), row=1, col=1)
