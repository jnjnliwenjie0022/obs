...
//
// Example:
//	wire   [6:0] in;	// 7'b0000100
//	wire   [2:0] out;	// 3'd2
//
//	atcaxi2tluh500_onehot2bin #(.N(7)) u_bin (.out(out), .in(in));
//
//
...
parameter N = 8;		// one-hot bit-width: N<=256
...
//
//:generate
//:if (W == $w) begin : gen_w${w}
//:	wire [$n:0] inx = {{(${n}+1-N){1'b0}}, in};
//:	wire nds_unused_ix = inx[$n];
//	for ($b = 0; $b < $w; $b++) {
//		my $tmp = "";
//		for ($j=0; $j<$n; $j++) {
//			if (($j/(1<<$b)) % 2) {
