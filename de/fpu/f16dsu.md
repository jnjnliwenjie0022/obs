...
- DIV cnt: \[0:q_bitwidth/2 - 1\]
- SQRT cnt:  \[1:q_bitwidth/2\ - 1]
	- din(f16): 11 (1+10)
		- DIV cnt: \[0:6\]
		- SQRT cnt:  \[1:6\]
	- din(f32): 24 (1+23)
	...
	- $initialize$
		...
		- $2>x>=1$
			...
			- $if\ exp\ is\ odd\ then\ x'=x/2$
				- $1>x'=4*r0>=0.5$
			![[(DSU)Low Latency Floating-Point Division and Square Root Unit.pdf#page=4&rect=306,402,576,735|(DSU)Low Latency Floating-Point Division and Square Root Unit, p.4|500]]
		- $1>d>=0.5$
...
- $residual\ recurrence\ with\ radix\ 4\ by\ using\ on\ the\ fly\ conversion\ into\ conventional\ representation$
	...
	- $r(j)=4*r(j-1)-X$
		- 帶入on-the-fly redundant conversion的公式
			- $qm(j)=q(j)-4^{-j}$![[Pasted image 20250428123204.png]]
