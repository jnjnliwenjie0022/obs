...
//VPERL_BEGIN
//for ($aw=9; $aw<=17; $aw++) {
//	foreach $dw (32, 39, 64, 72) {
//		if($aw==9 && $dw==32) {
//:localparam INVALID_RAM = ~(((ILM_RAM_AW==%d) && (ILM_RAM_DW==%d)) || :: $aw, $dw
//		}
//		elsif ($aw==17 && $dw==72) {
//:                            ((ILM_RAM_AW==%d) && (ILM_RAM_DW==%d))) ; :: $aw, $dw
//		}
//		else {
//:                            ((ILM_RAM_AW==%d) && (ILM_RAM_DW==%d)) || :: $aw, $dw
//		}
...
//
//:generate
//:if ((ILM_RAM_AW==%d) && (ILM_RAM_DW==%d)) begin : gen_ram%dx%d	:: $aw, $dw, 1<<$aw, $dw
//:	ram%dx%d ram_inst (				:: 1<<$aw, $dw
//:		.clka	(clk),
//:		.ena	(ilm_cs),
//if ($dw == 39) {
//:		.wea	(ilm_word_we),
...
if (INVALID_RAM) begin : gen_error
	// Xilinx Vivado does not support memory size > 512 KiB
