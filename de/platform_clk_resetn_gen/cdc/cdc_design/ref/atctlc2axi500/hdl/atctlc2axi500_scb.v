...
`ifdef ATCTLC2AXI500_FORCE_ID_REMAP
localparam FORCE_ID_REMAP = 1;
`else
...
//localparam TL_OP_INTENT         = 3'd5;
localparam TL_OP_ACQUIREBLOCK   = 3'd6;
localparam TL_OP_ACQUIREPERM    = 3'd7;

//localparam TL_OP_PROBEBLOCK   = 3'd6;
//localparam TL_OP_PROBEPERM    = 3'd7;

localparam TL_OP_ACCESSACK      = 3'd0;
localparam TL_OP_ACCESSACKDATA  = 3'd1;
//localparam TL_OP_HINTACK      = 3'd2;
//localparam TL_OP_C_NOP        = 3'd3;
//localparam TL_OP_PROBEACK     = 3'd4;
...
localparam TL_PERM_2T      = 2'd0;//Cap
localparam TL_PERM_2B      = 2'd1;//Cap
localparam TL_PERM_2N      = 2'd2;//Cap
localparam TL_PERM_N2B     = 3'd0;//Grow
localparam TL_PERM_N2T     = 3'd1;//Grow
localparam TL_PERM_B2T     = 3'd2;//Grow
localparam TL_PERM_T2B     = 3'd0;//Prune
localparam TL_PERM_T2N     = 3'd1;//Prune
localparam TL_PERM_B2N     = 3'd2;//Prune
localparam TL_PERM_T2T     = 3'd3;//Report
localparam TL_PERM_B2B     = 3'd4;//Report
localparam TL_PERM_N2N     = 3'd5;//Report
...
wire               b_probackdata_drop;
wire               c_op_probeackdata;
reg [TL_IDLEN-1:0] c2x_is_probeackdatas;
`endif
...
`ifdef ATCTLC2AXI500_FORWARD_PROBEACKDATA
assign c_op_probeackdata = c_opcode == TL_OP_PROBEACKDATA;
assign b_probackdata_drop = c2x_is_probeackdatas[b2d_source] & c2w_busys[b2d_source];
`endif
...
assign d_valids = {rvalid, bvalid, a2d_valid, c2d_valid}
`ifdef ATCTLC2AXI500_FORWARD_PROBEACKDATA
                & {1'b1, ~b_probackdata_drop, 2'b11}
`endif
                ;
assign {rready, bready, a2d_ready, c2d_ready} = d_readys
`ifdef ATCTLC2AXI500_FORWARD_PROBEACKDATA
                                              | {1'b0,  b_probackdata_drop, 2'b00}
`endif
                                              ;
...
`ifdef ATCTLC2AXI500_FORWARD_PROBEACKDATA
for (i=0; i<TL_IDLEN; i=i+1) begin : gen_c2w_pbd_ent
    wire we;
    assign we = c2w_valid & c2w_ready & c_source_onehot[i];
    always @(posedge clk or negedge resetn) begin
        if (!resetn) begin
            c2x_is_probeackdatas[i] <= 1'b0;
        end
        else if (we) begin
            c2x_is_probeackdatas[i] <= c_op_probeackdata; 
        end
    end
end
`endif
