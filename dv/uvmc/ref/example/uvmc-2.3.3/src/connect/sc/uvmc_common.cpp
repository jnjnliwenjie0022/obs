...
int uvmc_proxy_base::x_nb_transport_bw (
...
// UVMC233_ADDITIONS {
  unsigned long long uvmc_proxy_base::x_trans_mm_alloc(){
      UVMCERR(x_trans_mm_alloc);
      return 0LL;
  }

  void uvmc_proxy_base::x_trans_mm_free( unsigned long long trans_chandle ){
      UVMCERR(x_trans_mm_free);
  }
// } UVMC233_ADDITIONS
...
int SV2C_nb_transport_bw (int id,
    unsigned long long trans_chandle,
    bits_t *bits, uint32 *phase, sc_dt::uint64 *delay)
{
  GET_PROXY_FROM_ID(SV2C_nb_transport_bw,tlm::TLM_COMPLETED)
  return proxy->x_nb_transport_bw(trans_chandle,bits,phase,delay);
}
...
// UVMC233_ADDITIONS {
unsigned long long SV2C_trans_mm_alloc( int id ){
    GET_PROXY_FROM_ID(SV2C_trans_mm_alloc,0LL)
    return proxy->x_trans_mm_alloc();
}
void SV2C_trans_mm_free( int id, unsigned long long trans_chandle ){
    GET_PROXY_FROM_ID_VOID(SV2C_trans_mm_free)
    proxy->x_trans_mm_free( trans_chandle );
}
// } UVMC233_ADDITIONS
...
// UVMC233_ADDITIONS {
//------------------------------------------
bool uvmc_is_trans_mm_enabled = false;

void uvmc_enable_trans_mm(){ uvmc_is_trans_mm_enabled = true; }

extern "C" {
svBit SV2C_is_trans_mm_enabled(){
    return uvmc_is_trans_mm_enabled == true; }
}
// } UVMC233_ADDITIONS
