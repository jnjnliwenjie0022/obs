#===========================================================================
# @(#) $Id: xlcheck.pl 2036 2023-08-13 05:41:56Z jstickle $
#===========================================================================
...
@toolVersions = (
    "questa 2020.4 2021.1 2021.2 2021.4 2022.1 2023.1",
    "gcc 4.9.1 5.3.0 6.3.0 7.4.0",
    "osciSysc 2.3.1 2.3.2 2.3.3",
    "vista 4.8.0 4.9.0 5.2.0",
    "virtualizer T-2022.06",
    "ius 13* 14*" );
    ...
    foreach $line (@toolVersions) {
        ...
        foreach $token (@versionList) {
            if( $tool eq "" ) { $tool = $token; }
                ...
                elsif( $tool eq "virtualizer" ){
                    if( $token =~ /([TN]-[0-9]*\.[0-9]*)/ ) {
                        $key = $1;
                        $virtualizerVersions{$key} = "valid";
                    }
                }
                elsif( $tool eq "ius" ){
                    if( $token =~ /([0-9]*)/ ) {
...
sub checkSysc() {
    if( $toolsToCheck{"env_sysc"} eq "yes" ) {
        $osciSyscBanner = "$ENV{SYSTEMC}";
        if( $osciSyscBanner =~ /systemc-([0-9]\..\..)/ ) {
            $osciSyscVersion = $1; }

        if( $osciSyscVersion eq "" ) {
            if( $osciSyscBanner =~ /systemc_([0-9])([0-9])([0-9])/ ) {
            $osciSyscVersion = "$1.$2.$3"; } }
...
#-----------------------------------------------------------------------------
# Check Virtualizer environent (env_virtualizer)
#-----------------------------------------------------------------------------

sub checkVirtualizer() {
    if( $toolsToCheck{"env_virtualizer"} eq "yes" ) {
        #$virtualizerBanner = `vs -version`;
        $virtualizerBanner = "T-2022.06";
        if( $virtualizerBanner =~ /([TN]-[0-9]*\.[0-9]*)/ ) { $virtualizerVersion = $1; }

        if( $virtualizerVersion ne "" && $virtualizerVersions{$virtualizerVersion} ne "valid" ){
            printToolVersions();
            die "ERROR: Incompatible Virtualizer release: $virtualizerBanner.\n"; }
        print "virtualizer$virtualizerVersion\n";
    }
}
