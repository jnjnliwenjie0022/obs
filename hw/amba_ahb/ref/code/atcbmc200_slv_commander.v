...
// &FORCE("input","hclk");
// &FORCE("input","hresetn");
// &FORCE("input","init_priority[15:0]");
// &FORCE("input","mst0_highest_en");
// &FORCE("input","hs_hresp[1:0]");
//
// for($j = 0; $j < $MST_NUM; ++$j) {
//   &IFDEF("ATCBMC200_AHB_MST${j}");
//   &FORCE("output","mst${j}_ack");
//   &FORCE("input","mst${j}_req");
...
// }
//
// &FORCE("output","hs_haddr[ADDR_MSB:0]");
// &FORCE("output","hs_hburst[2:0]");
// &FORCE("output","hs_hprot[3:0]");
// &FORCE("output","hs_hsize[2:0]");
// &FORCE("output","hs_htrans[1:0]");
// &FORCE("output","hs_hwrite");
// &FORCE("output","hs_hwdata[DATA_MSB:0]");
// &FORCE("output","hs_hsel");
// &FORCE("output","bmc_hs_hready");
// &FORCE("input","hs_bmc_hready");
//
// &FORCE("wire","hs_hmaster[3:0]");
// &FORCE("wire", "slv_sel");
//
...
// for($j = 0; $j < $MST_NUM; ++$j) {
//: `ifdef ATCBMC200_AHB_MST${j}
//:                           (init_priority[${j}] & next_priority${j}) |
//: `endif
// }
//:                           1'b0 );
//:
//: // Assert arbitration when
//: // 1. no active transactions
//: // 2. a taken master request completes or a new transaction from the same master is coming
//: assign arb_valid = ((~mst_valid) | (bmc_hs_hready & (hs_hresp == HRESP_OK))) & (
// for($j = 0; $j < $MST_NUM; ++$j) {
//: `ifdef ATCBMC200_AHB_MST${j}
//:                           ((mst_id == 4'd${j}) & (mst${j}_htrans == HTRANS_NONSEQ | mst${j}_htrans == HTRANS_IDLE) ) |
//: `endif
// }
//:                           1'b0 );
//:
...
//: // Assert hs$j_hsel and pass the related transaction signals when
//: // 1. the 1st cycle of a transaction (arb_valid == 1)
//: // 2. the rest cycles of a burst transfer
//: assign slv_sel      = (arb_valid) ? mst_req : (
// for($j = 0; $j < $MST_NUM; ++$j) {
//: `ifdef ATCBMC200_AHB_MST${j}
//:                           ((mst_id == 4'd${j}) & mst_valid & ((mst${j}_htrans == HTRANS_SEQ) | (mst${j}_htrans == HTRANS_BUSY))) |
//: `endif
// }
//:                           1'b0 );
...
//:     `ifdef ATCBMC200_AHB_MST${j}
//:        {1'b1, 4'd${j}}: begin
//:        hs_htrans = mst${j}_htrans;
//:        hs_haddr  = mst${j}_haddr;
//:        hs_hwrite = mst${j}_hwrite;
//:        hs_hsize  = mst${j}_hsize;
//:        hs_hburst = mst${j}_hburst;
//:        hs_hprot  = mst${j}_hprot;
//:     end
//:     `endif
// }
//:     default: begin
...
//:    4'd0;
//:
//: // Sample the current master request when 
//: // 1. arbitration cycle
//: // 2. the 2nd cycle of an error response
//: always @(posedge hclk or negedge hresetn) begin
//:     if (!hresetn)
//:         mst_valid <= 1'b0;
...
// for($j = 0; $j < $MST_NUM; ++$j) {
//: `ifdef ATCBMC200_AHB_MST${j}
//:         priority${j} <= next_priority${j} | (priority_reload & init_priority[${j}]);
//: `endif
// }
//:     end
//: end
...
// for($j = 1; $j < $MST_NUM; ++$j) {
//: `ifdef ATCBMC200_AHB_MST${j}
//:     assign mask${j}_req = priority${j} & mst${j}_req;
//: `endif
// }
//:
// for($j = 0; $j < $MST_NUM; ++$j) {
//: `ifdef ATCBMC200_AHB_MST${j}
//:     assign lv_mask${j}_req = init_priority[${j}] & mask${j}_req;
//: `endif
// }
//:
//: assign pri_sel =
...
//         for($j = 0; $j < $MST_NUM; ++$j) {
//:        `ifdef ATCBMC200_AHB_MST${j}
//:            if (lv_mask${j}_req) mst${j}_ack = 1'b1; else
//:        `endif
//         }
//         for($j = 0; $j < $MST_NUM; ++$j) {
//:        `ifdef ATCBMC200_AHB_MST${j}
//:            if (mask${j}_req) mst${j}_ack = 1'b1; else
//:        `endif
//         }
//:        begin
//             for($j = 0; $j < $MST_NUM; ++$j) {
...
//         for($j = 0; $j < $MST_NUM; ++$j) {
//:        `ifdef ATCBMC200_AHB_MST${j}
//:            if (mst${j}_req) mst${j}_ack = 1'b1; else
//:        `endif
//         }
//:        begin
//             for($j = 0; $j < $MST_NUM; ++$j) {
...
//         for($j = 0; $j < $MST_NUM; ++$j) {
//:        `ifdef ATCBMC200_AHB_MST${j}
//:            next_priority${j} = mask${j}_req;
//:        `endif
//         }
//:
//         for($j = 0; $j < $MST_NUM; ++$j) {
//:        `ifdef ATCBMC200_AHB_MST${j}
//:            if (lv_mask${j}_req) next_priority${j} = 1'b0; else
//:        `endif
//         }
//         for($j = 0; $j < $MST_NUM; ++$j) {
//:        `ifdef ATCBMC200_AHB_MST${j}
//:            if (mask${j}_req) next_priority${j} = 1'b0; else
//:        `endif
//         }
//:        begin
//             for($j = 0; $j < $MST_NUM; ++$j) {
//:            `ifdef ATCBMC200_AHB_MST${j}
//:                next_priority${j} = 1'b0;
//:            `endif
//             }
//:        end
//:    end
...
//         for($j = 0; $j < $MST_NUM; ++$j) {
//:        `ifdef ATCBMC200_AHB_MST${j}
//:            next_priority${j} = mst${j}_req;
//:        `endif
//         }
//:
//         for($j = 0; $j < $MST_NUM; ++$j) {
//:        `ifdef ATCBMC200_AHB_MST${j}
//:            if (mst${j}_req) next_priority${j} = 1'b0; else
//:        `endif
//         }
//:        begin
//             for($j = 0; $j < $MST_NUM; ++$j) {
...
assign hs_hmaster   = (arb_valid & mst_req) ? next_mst_id : mst_id;
assign hs_hsel      = slv_sel;
// Assert hs and pass the related transaction signals when
// 1. the 1st cycle of a transaction (arb_valid == 1)
// 2. the rest cycles of a burst transfer
assign slv_sel      = (arb_valid) ? mst_req : (
`ifdef ATCBMC200_AHB_MST0
